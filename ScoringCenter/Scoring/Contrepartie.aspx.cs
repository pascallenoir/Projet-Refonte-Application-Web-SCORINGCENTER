...
namespace ScoringCenter.Scoring
{
    public partial class Contrepartie : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            TxtBanque.Text = service.InfosBanqueByCode(code_banque).NOM_BANQUE;
            if (!IsPostBack)
            {
                vider();
            }
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            DdlBranchActiv.SelectedValue = "A1";
            SecteurActivite();
        ...
        public static string[] AjouterFiliale(string text)
        {
            ...
            if (get_AjouterFiliale.Count != 0)
            {
                ...
                {
                    var lecode_banque = "";
                    foreach (var banque in service.InfoBanque(v.CODE_AGENCE))
                    {
                        lecode_banque = banque.CODE_BANQUE;
                    }
                    if (lecode_banque.Trim() == code_banque.Trim())
                    {
        ...
        protected void ValideContrepartie_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ...
                var banque = Session["code_banque"].ToString();

                string mode = "";

                if (Nom.Value == "" || DdlStatut.Value == "" || TbPays.Value == "" /*|| Ca.Value == ""*/ || Adresse.Value.Equals("") || Ville.Value.Equals("")
                || Rccm.Value.Equals("") || DdlSecteurActive1.SelectedValue.Equals("") || DdlStatut.Value.Equals("") || DdlAgence.Value.Equals("") 
                || (DdlGroup.SelectedValue.Equals("OUI") && Tbgroupe.Value.Equals("")))
                {
                    OpenShowPasvalideConsult("Erreur...", "Un champ obligatoire n'a pas été renseigné !!!");
                    return;
                }

                //if (G_oui.Checked)
                if (DdlGroup.Text.Equals("OUI"))
                {
                    //R_non.Checked = false;
                    //R_oui.Checked = false;
                    ...
                    //else
                    //{
                        var Countelements = service.CONTRO_CONTREPARTIE(Nom.Value.Trim());

                        if (Countelements.Count() > 0)
                        {
                            OpenShowPasvalideConsult("Erreur...", "La Raison sociale renseignée existe déjà !!!");
                        }
                        else
                        {
                            service.InsertContrepartieGroup(id_user.Trim(), DdlAgence.Value.Trim(), "", Nom.Value.Trim(), Adresse.Value.Trim(), Ville.Value.Trim(), Rccm.Value.Trim(), Segment.Value.Trim(), mode.Trim(), TypeAnalyse.Value.Trim(), DblUntie.Value, DeviseD.SelectedValue.Trim(), Convert.ToDecimal(Ca.Value.Trim().Replace(",", "").Replace(" ", "")), TbPays.Value.Trim(), DdlStatut.Value.Trim(), TbActivite1.Value, DdlSecteurActive1.SelectedValue.Trim(), Tbgroupe.Value.Trim(), DdlBranchActiv.SelectedValue.ToString().Trim());
                ...
                else
                {
                    ...
                    //if (Nom.Value == "" || DdlStatut.Value == "" || TbPays.Value == "" || Ca.Value == "" || DeviseD.SelectedValue == "")
                    //{
        ...
        public void BrancheActivite()
        {
            ...
            if (branchActivite.Count != 0)
            {
                ...
                {
                    DdlBranchActiv.Items.Add(new ListItem(v.BRANCH_ACT_LIBELLE, v.BRANCH_ACT_CODE.Trim()));
                }
            }
        }
