...
<%--                <div  class="col-md-10" style="width:90%; height:100%; background-color:blue" >
                                ...
                                <div class="row" style="margin-bottom: 2%; text-align: left;">
                                    ...
                                    <div class="col-lg-8 col-sm-8 col-md-8">
                                        <div style="display: inline; text-align: left; margin-left: 80%;">
                                            ...
                                            <asp:TextBox ID="TbCodeProilRemote" runat="server" CssClass="form-control Ddlsize display" Width="40"></asp:TextBox>
                                            <asp:TextBox ID="TbPlafondRemote" runat="server" CssClass="form-control Ddlsize display" Width="40"></asp:TextBox>
                                    ...
                                    <div class="col-lg-12 col-sm-12 col-md-12">
                                        <div class="row push_entete">
                                            ...
                                            <div class="col-lg-3 col-sm-3 col-md-3">
                                                <asp:DropDownList ID="DdlBanque" AutoPostBack="true" onchange="combo007($(this),'C')"  CssClass="form-control" runat="server" OnSelectedIndexChanged="OnBanqueChangeLoadPlafonds" style="height:24px;padding-top:0px;padding-bottom:0px; width: 110px" />
                                            </div>
                                        </div>
                                        <div class="row push_entete">
                                            <div class="col-lg-3 col-sm-3 col-md-3 col-lg-offset-1 col-sm-offset-1 col-md-offset-1" 
                                            ...
                                            <div class="col-lg-3 col-sm-3 col-md-3">
                                                <asp:TextBox ID="TbNLibelleP" runat="server" MaxLength="50" onchange="con007($(this),'T')" CssClass="form-control" 
                                        ...
                                        <div class="row push_entete">
                                            ...
                                            <div class="col-lg-2 col-sm-2 col-md-2">
                                                <asp:TextBox ID="TbICodePro" runat="server" CssClass="form-control Uper"   onchange="con007($(this),'T')"
                                        ...
                                        <div class="row push_entete">
                                            ...
                                            <div class="col-lg-3 col-sm-3 col-md-3">
                                                ...
                                                <select runat="server" id="TbNPlafond" class="form-control" onchange="combo007($(this),'T')" style="height: 24px; padding-top: 0px; padding-bottom: 0px; width: 300px;">
                                                    <option></option>
    ...
    <script>
        ...
        $('#<%=DdlBanque.ClientID%>, #<%=TbICodePro.ClientID%>, #<%=TbNPlafond.ClientID%>, #<%=TbNLibelleP.ClientID%>, #<%=EtatProfil.ClientID%>').on('change', function () {
            $('#<%=TbCodeBanqueRemote.ClientID%>').val($('#<%=DdlBanque.ClientID%>').val());
            $('#<%=TbCodeProilRemote.ClientID%>').val($('#<%=TbICodePro.ClientID%>').val());
            $('#<%=TbPlafondRemote.ClientID%>').val($('#<%=TbNPlafond.ClientID%>').val());
            $('#<%=btnValider.ClientID%>').show('fast');
        });

        function chargerchamptxt(param) {
            var code_profil = param.closest('tr').find('td').eq(0).html();
            var code_banque = param.closest('tr').find('td').eq(1).attr('Data').replace(/&nbsp;/g, '').replace(',', '');
            var libelle_profil = param.closest('tr').find('td').eq(2).html();
            var plafond_profil = param.closest('tr').find('td').eq(3).attr('Data').replace(/&nbsp;/g, '').replace(',', '');
            var statutval = param.closest('tr').find('td').eq(4).html();
            var id_profil = param.closest('tr').find('td').eq(5).html();

            var chaine1 = "";
            chaine1 = chaine1 + "SC#G:" + libelle_profil;
            O42451(chaine1);

            $('#<%=btnValider.ClientID%>').hide('fast');

            $('#<%=TbICodePro.ClientID%>').prop("readonly", true);
            $('#<%=DdlBanque.ClientID%>').removeAttr("disabled", "disabled");

            $.ajax({
                type: "POST",
                url: "Profil.aspx/LoadPlafondSelectOptionsHtml",
                data: "{'code_banque': '" + code_banque + "'}",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                beforeSend: function () { },
                success: function (response) {
                    $('#<%=TbNPlafond.ClientID%>').html(response.d._plafonds);
                    ...
                    if (statutval === '<div class="circle_green"></div>') document.getElementById('<%=EtatProfil.ClientID%>').checked = true;
                    else document.getElementById('<%=EtatProfil.ClientID%>').checked = false;
                },
