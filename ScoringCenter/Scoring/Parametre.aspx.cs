...
namespace ScoringCenter.Scoring
{
    public partial class Parametre : System.Web.UI.Page
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            decimal plafond = 0; // service.SELECT_MAJ_PLAFON().SingleOrDefault().PLAFOND;
            PlafondCA.Value = plafond.ToString();
            //SectAct.Disabled = false;
            //CodeAct.Disabled = false;
            //FormeJuri.Disabled = false;
        ...
        public void ListeProfil()
        {
            ...
            DblProfil.Items.Add("");
            var liste_banques = service.ListeBanque(ScorCryptage.Decrypt(param));
            foreach(var element in liste_banques)
            {
                var i_typeEvaluation = service.ListeUser(2, "", "", "", "", "", "", "", "", DateTime.Now, true, element.CODE_BANQUE, "");
                foreach (var v in i_typeEvaluation)
                {
                    ListItem l = new ListItem();
                    l.Value = v.ID_PROFIL;
                    l.Text = element.SIGLE_BANQUE + " " + v.LIBELLE_PROFIL;
            ...
            if (id_profil.Trim() == "0")
            {
                ...
                if (Vdroit.Count > 0)
                {
                    ...
                    foreach (var v in Vdroit)
                    {
                        ...
                        if (v.ID_DE_REGROUPEMENT.Trim().Length >= 2)
                        {
                            sb.AppendLine("<tr onclick=\"ClickCheckbox($(this))\" style='background-color:#fdf5e6' class='clickable'>");
                        ...
                        {
                            sb.AppendLine("<tr onclick=\"ClickCheckbox($(this))\" style='background-color:#f0f8ff' class='clickable'>");
                            sb.AppendLine(string.Format("<td class='text-left' style='display:none'>{0}</td>", v.ID_DROIT.Trim()));
                            sb.AppendLine(string.Format("<td  style='padding-left:5%'>{0}</td>", v.LIBELLE_DROIT));
                            sb.AppendLine(string.Format("<td  class='text-center'><input name='selectM' runat='server' type='checkbox' id='M' class='singlechkbox caseACocher M_Cocher{0}' codeM=" + v.ID_DROIT + "/></td>", k));
                            sb.AppendLine(string.Format("<td  class='text-center' ><input name='selectV' runat='server' type='checkbox' id='V' class='singlechkbox caseACocher V_Cocher{0}' codeV=" + v.ID_DROIT + "/></td>", k));
        ...
        protected void DblProfil_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Vdroit.Count > 0)
            {
                ...
                foreach (var v in Vdroit)
                {
                    if (v.ID_DE_REGROUPEMENT.Trim().Length >= 2)
                    {
                        sb.AppendLine("<tr class='clickable'  style='background-color:#fdf5e6'>");
                        sb.AppendLine(string.Format("<td class='text-left' style='display:none'>{0}</td>", v.ID_DROIT.Trim()));
                        sb.AppendLine(string.Format("<td  style='padding-left:5%'>{0}</td>", v.LIBELLE_DROIT));
                        ...
                        sb.AppendLine("<tr class='clickable' style='background-color:#f0f8ff'>");
                            sb.AppendLine(string.Format("<td class='text-left' style='display:none'>{0}</td>", v.ID_DROIT.Trim()));
                            sb.AppendLine(string.Format("<td  style='padding-left:5%'>{0}</td>", v.LIBELLE_DROIT));
        ...
        protected void ValideModele_ServerClick(object sender, EventArgs e)
        {
            if (PlafondCA.Value == "")
            {
            ...
            {
                //service.ENREGISTREMENT_MAJ_PLAFON(Convert.ToDecimal(PlafondCA.Value.Trim().Replace(" ", "")));
                OpenShowPasvalideConsult("Modification", "Modification effectuée avec succès !!!");
            }
