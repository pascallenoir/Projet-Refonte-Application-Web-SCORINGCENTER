    ...
    public partial class FicheSignaletique : System.Web.UI.Page
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string code_banque;
                try { code_banque = ScorCryptage.Decrypt(Session["code_banque"].ToString());}
                catch (Exception) { code_banque = Session["code_banque"].ToString(); }
                Session.Add("code_banque", code_banque);

                if (Session["id_scoring"] == null) Session.Add("id_scoring", ScorCryptage.Decrypt(Request.QueryString["id"]));
                if (string.IsNullOrEmpty(Session["id_scoring"].ToString()))
                    Response.Redirect("~/Scoring/AutreDossier.aspx");
            }

            updateCaModeleNotation();
            getInfoClient();

        }

        public void updateCaModeleNotation() {
            if (Session["code_banque"] == null) Response.Redirect("~/Scoring/Connexion.aspx");

            string code_banque = Session["code_banque"].ToString();
            string id_scoring = Session["id_scoring"].ToString();

            PS_VSCOR_FICHESIGNALETIQUEResult dossier = service.DetailsDossierContrepartie(code_banque, id_scoring).SingleOrDefault();
            string id_dossier = dossier.ID_DOSSIER;
            Session.Add("id_dossier", id_dossier);

            decimal ca = 0;

            // récupérer les liasses du dossier concerné
            List<PS_VSCOR_AFFICHER_LIASSEResult> liasses = service.AnafiAfficheLiasse(7, id_dossier, "", "");
            if (liasses.Count != 0) {
                // récupérer le CA
                string annee_detail = liasses.LastOrDefault().ANNEE_DETAIL;
                ca = Convert.ToDecimal(service.DetailsDossierLiasseContrepartie(1, id_dossier, annee_detail).FirstOrDefault().CA);
            }
            
            // déterminer le modèle de notation
            PS_SCOR_MODELE_NOTATIONResult modele = service.DeterminerModeleNotation(dossier.SEGMENT_CLIENT, ca, dossier.SECTEUR_D_ACTIVITE, dossier.STATUT);

            // mettre à jour CA et modèle de notation pour la contrepartie et le dossier
            string code_modele = modele != null ? modele.CODE_MODELE : "INDTER";
            service.MajCaModeleNotationContrepartie(dossier.ID_SCORING, code_modele, ca);
            service.MajModeleNotationDossier(id_dossier, code_modele);
        }
        ...
        public void verifses()
        {
            ses.InnerHtml = "<script> $(\"#<%=ReconnexionOpenConsult.ClientID%>\").click();</script>";
        }
        ...
        public void modifierFiche(object sender, EventArgs e)
        {
            ...
            if (DeviseD.SelectedValue == "" || Nom.Value == "" || paysLab.Value == "" || Ca.Value == "" || TbActivite1.Value == "" || DeviseD.SelectedValue == "" || DdlStatut.Value == "")
            {
            ...
            else
            {
                if (Id_SCOR.Trim() != "SCG")
                {
                    ...
                    service.UpdateContrepartiemd(ISCenter.Text.ToString().Trim(), IPrincipal.Text.ToString(), Nom.Value.ToString(),
                        ...
                        paysLab.Value.ToString(), Segment.Value.ToString(), DdlAgence.Value, Convert.ToDecimal("0" + Ca.Value.ToString().Replace("&nbsp;", "").Trim().Replace(" ", "")),
                        DeviseD.SelectedValue.ToString(), Rccm.Value.ToString(), DdlSecteurActive1.SelectedValue.ToString(),
                        TbActivite1.Value.ToString(), DdlStatut.Value, "", DblUntie.Value.ToString(), Tbgroupe1.Value.ToString().Trim(), DdlBranchActiv.SelectedValue.ToString().Trim());
                }
                else
                {
                    service.UpdateContrepartiemd(ISCenter.Text.ToString().Trim(), IPrincipal.Text.ToString(), Nom.Value.ToString(),
        ...
        public void ListeTableParam()
        {
            ...
            if (Session["code_banque"] != null)
            {
                ...
                foreach (var v in service.ListeAgenceBanque(code_banque))
                {
        ...
        public void BrancheActivite()
        {
            ...
            if (branchActivite.Count != 0)
            {
                ...
                {
                    if (v.BRANCH_ACT_LIBELLE.Trim() == LblBranche.Text.ToString().Trim())
                    {
                        DdlBranchActiv.Items.Add(new ListItem(v.BRANCH_ACT_LIBELLE.Trim(), v.BRANCH_ACT_CODE.Trim(), true));
                    }
                    else
                    {
                        DdlBranchActiv.Items.Add(new ListItem(v.BRANCH_ACT_LIBELLE.Trim(), v.BRANCH_ACT_CODE.Trim()));
        ...
        public void SecteurActivite()
        {
            ...
            var ActiviteBCEO = service.SecteurActivite("A1");
        ...
        public void autoselectcombo(object sender, EventArgs e)
        {
            ...
            foreach (var v in info)
            {
                ...
                DdlSecteurActive1.SelectedIndex = DdlSecteurActive1.Items.IndexOf(DdlSecteurActive1.Items.FindByValue(v.SECTEUR_D_ACTIVITE.Trim()));
                DdlSecteurActive1_SelectedIndexChanged(sender, e);
                ...
                //Eloi 06/02/2023
                DdlAgence.SelectedIndex = DdlAgence.Items.IndexOf(DdlAgence.Items.FindByValue(v.CODE_AGENCE));
                DdlSecteurActive1.SelectedIndex = DdlSecteurActive1.Items.IndexOf(DdlSecteurActive1.Items.FindByValue(v.SECTEUR_D_ACTIVITE.Trim()));
            ...
            if (string.IsNullOrEmpty(Request.QueryString["id"]))
            {
                ...
                else
                {
                    ...
                    foreach (var v in info)
                    {
                        ...
                        Pays.Text = v.PAYS;
                        IPrincipal.Text = v.TYPE_PROSPECT.Equals("Prospect") ? v.ID_SCORING : v.ETCIV_MATRICULE;
                        ...
                        MAnalyse.Text = AnalyseFinanciere.getModeleNotation(v.ID_DOSSIER.Trim()); 
                        //if (v.LIBELLE_MODELE.Trim() == "Groupe") AQ.Visible = false; else AS.Visible = true;

                        //Eloi 06/02/2023
                        lblModelAnalP2.Text = AnalyseFinanciere.getModeleNotation(v.ID_DOSSIER.Trim());
                        lblSectionActiv.Text = v.SECTACT_LIBELLE;
                        ...
                        LblChiffre.Text = v.CA != 0 ? Convert.ToString(string.Format(frCult, "{0:#,##0}", Convert.ToDecimal("0" + v.CA))) : "NC";
                        lblCA.Text = v.CA != 0 ? Convert.ToString(string.Format(frCult, "{0:#,##0}", Convert.ToDecimal("0" + v.CA))) : "NC";
                        ...
                        //Eloi 03/02/2023
                        lblIdentifFiscal.Text = "NC";
                        lblIdentifPrincipalP2.Text = v.TYPE_PROSPECT.Equals("Prospect") ? v.ID_SCORING : v.ETCIV_MATRICULE;
                        lblIdentifScorP2.Text = v.ID_SCORING;
                        lblTypeClientP2.Text = v.TYPE_PROSPECT;
                        lblBanque.Text = v.CODE_BANQUE == null ? "NC" : service.InfosBanqueByCode(v.CODE_BANQUE).NOM_BANQUE.ToUpper();
                        lblBanqueP2.Text = v.CODE_BANQUE == null ? "NC" : service.InfosBanqueByCode(v.CODE_BANQUE).NOM_BANQUE.ToUpper();
                        //Tbgroupe.Value = v.GROUPE_DOSSIER;
                        ...
                        Ca.Value = Convert.ToString(string.Format(frCult, "{0:#,##0}", Convert.ToDecimal("0" + v.CA)));
                        //Eloi 03/02/2023
                        lblCAP2.Text = v.CA != 0 ? Convert.ToString(string.Format(frCult, "{0:#,##0}", Convert.ToDecimal("0" + v.CA))) : "NC";
                        Rccm.Value = v.RCCM;

                        if (v.GROUPE_DOSSIER != "" && v.GROUPE_DOSSIER != null) { }
                        else { IG.Visible = false; }
                    }
                    var infoLiass = service.DetailsDossierLiasseContrepartie(0, id_dossier.Trim(), "");
                    foreach (var v in infoLiass)
                    {
                        LBDateClotureLiasse.Text = "NC";
                        if (v.DATE_CLOTURE != null || Convert.ToDateTime(v.DATE_CLOTURE).ToString("dd/MM/yyyy") != "01/01/1900")
                        {
                            LBDateClotureLiasse.Text = Convert.ToDateTime(v.DATE_CLOTURE).ToString("dd/MM/yyyy");
                            //Eloi 03/02/2023
                            lblDateClotP2.Text = Convert.ToDateTime(v.DATE_CLOTURE).ToString("dd/MM/yyyy");
                        }
                        LBDurreExoMoisLiasse.Text = "NC";
                        if (v.DUREE_EXERCICE_MOIS != null)
                        {
                            LBDurreExoMoisLiasse.Text = v.DUREE_EXERCICE_MOIS.ToString();
                            //Eloi 03/02/2023
                            lblDureeExerP2.Text = v.DUREE_EXERCICE_MOIS.ToString();
                        }
                        LBTypeBilLiass.Text = "NC";
                        // if (v.TYPE_ANAFI_A != null) LBTypeBilLiass.Text = v.TYPE_ANAFI_A.ToString();
                        if (v.TYPE_ANAFI_A != null)
                        {
                            if (v.TYPE_ANAFI_A == "SMT")
                            {
                                LBTypeBilLiass.Text = "Systeme Minimal De Tresorerie";
                                //Eloi 03/02/2023
                                lblTypeBilanP2.Text = "Systeme Minimal De Tresorerie";
                            }
                            if (v.TYPE_ANAFI_A == "SN")
                            {
                                LBTypeBilLiass.Text = "Bilan Normal";
                                //Eloi 03/02/2023
                                lblTypeBilanP2.Text = "Bilan Normal";
                            }
                            if (v.TYPE_ANAFI_A == "SA")
                            {
                                LBTypeBilLiass.Text = "Bilan Allege";
                                //Eloi 03/02/2023
                                lblTypeBilanP2.Text = "Bilan Allege";
                            }
                        }
                        LBNatureExoLiasse.Text = "NC";
                        if (v.NATURE_EXERCICE != null)
                        {
                            LBNatureExoLiasse.Text = v.NATURE_EXERCICE.ToString();
                            //Eloi 03/02/2023
                            lblNatureExercP2.Text = v.NATURE_EXERCICE.ToString();
                        }
            ...
            else
            {
                ...
                foreach (var v in info)
                {
                    ...
                    if ((v.TYPE_PROSPECT).ToLower() == "prospect")
                    {
                        ...
                        SearchPays.Disabled = false;
                        DdlAgence.Disabled = false;
                        DdlSecteurActive1.Enabled = true;
                        DdlStatut.Disabled = false;
                        DeviseD.Enabled = true;
                    }
                    else
                    {
                        //Button1.Visible = false;
                        ...
                        adr.Disabled = true;
                        Ville.Disabled = true;
                        SearchPays.Disabled = true;
                        DdlAgence.Disabled = true;
                        DdlSecteurActive1.Enabled = false;
                        DdlStatut.Disabled = true;
                        DeviseD.Enabled = false;
                    }
        ...
        protected void DdlBranchActiv_SelectedIndexChanged1(object sender, EventArgs e)
        {
            DdlSecteurActive1.Items.Clear();
            SecteurActivite();
        }
